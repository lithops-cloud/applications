...
def write_command(backend, bucket_name, mb_per_file, number, key_prefix, outdir, name):
    if bucket_name is None:
        raise ValueError('You must provide a bucket name within --bucket_name parameter')
    res_write = write(backend, bucket_name, mb_per_file, number, key_prefix)
...
def read_command(backend, key_file, number, outdir, name, read_times):
    if key_file:
    ...
    res_read = read(backend, bucket_name, number, keynames, read_times)
...
@click.option('--name', default='os_benchmark', help='filename to save results in')
@click.option('--read_times', default=1, help="number of times to read each COS key")
def run(backend, bucket_name, mb_per_file, number, key_prefix, outdir, name, read_times):
    if True:
